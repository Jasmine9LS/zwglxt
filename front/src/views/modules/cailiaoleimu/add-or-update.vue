<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="材料类目" prop="cailiaoleimu">
          <el-input v-model="ruleForm.cailiaoleimu" 
              placeholder="材料类目" clearable  :readonly="ro.cailiaoleimu"></el-input>
        </el-form-item>
        <div v-else>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `cailiaoleimu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.cailiaoleimuCrossAddOrUpdateFlag = false;
                  this.parent.search();
